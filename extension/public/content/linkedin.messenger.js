...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.cid === "openai") {
        ...
        // Person's Name
        const links = document.querySelectorAll('a.msg-conversation-listitem__link')
        const id = window.location.href.replace(/.*thread|\//gmi, '');
        let name = ". .";
        for (const a of links) {
            if (a.href.indexOf(id) !== -1) name = a.querySelector('h3').textContent.replace("\n", '').trim();
        }

        const last_name = name.split(" ")[0] ? name.split(" ")[0] : name;
        const first_name = name.split(" ")[1] ? name.split(" ")[1] : name;
...
async function setupPopup() {
    ...
    for (const campaign of responses) {
        ...
        button.onclick = () => {
            ...
            const conversation = document.querySelectorAll('.msg-s-message-list__event');
            for (const div of conversation) {
                if (div.querySelector('img')) {
                    const title = div.querySelector('img').getAttribute('title');
                    const message = div.querySelector('p').textContent.trim();
                    const you = title.replace(/\s/gmi, '') !== name.replace(/\s/gmi, '');
