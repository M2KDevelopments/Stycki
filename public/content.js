...
chrome.storage.sync.get('active', data => {
    ...
    buttonCloseAll.onclick = async () => {
        ...
        if (result) {
            ...
            await chrome.storage.sync.set({ notes: remainingNotes });
...
chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
    if (message.cid === "alert") {
        Toastify({
            ...
            duration: 3000,
            destination: "https://m2kdevelopments.com",
            newWindow: true,
            close: true,
...
async function createNote(id, defaultNote = null, disabled = false) {
    let { notes } = await chrome.storage.sync.get('notes');
    if (!notes) notes = [];

    const { count } = await chrome.storage.sync.get('count');
