...
export default async function handler(req, res) {
    try {
        ...
        const { ids, title, message, url, icon, image, from, button1, button2 } = req.body;
        if (!ids) return res.status(400).json({ message: "No GCM tokens", result: false });
        const notifications = await Notificaition.find({ _id: { $in: ids } });
